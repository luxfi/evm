...
func (vm *VM) onNormalOperationsStarted() error {
	...
	// Start converged: until the pool publishes its first reorg event the two hashes
	// must already agree, or the barrier below would hold the builder closed forever.
	head := vm.blockChain.CurrentBlock().Hash()
	vm.builder.setChainHeadHash(head)
	vm.builder.setMempoolHeadHash(head)
