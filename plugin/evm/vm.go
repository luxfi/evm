...
func (vm *VM) Initialize(ctx context.Context, init block.Init) error {
	...
	if vm.config.ImportChainData != "" {
		...
		if err != nil {
			// IDEMPOTENCY GUARD: the flag-day / production pod spec keeps --import-chain-data
			// SET, so every restart AND every kill-rejoin (G9) re-enters this path. Once the
			// chain is already at/above the file's tip, importBlocksFromFile imports 0 blocks and
			// returns a "nothing to import" error (see isNothingToImportError) — that is SUCCESS
			// on a re-run, NOT a fatal boot error. Without this guard the node CRASH-LOOPS on
			// every restart-with-flag (killing G9 rejoin and every watchdog restart). On this
			// skip path chain.State was already restored to the correct tip by initChainState (it
			// read the persisted acceptedBlockDB on this boot), so nothing more is needed; and we
			// must NOT run reconcileQuasarWithAccept(lastHeight==0) here (it would wrongly reset
			// the export frontier).
			...
			// CONSENSUS-REFRESH: sync the IN-MEMORY consensus last-accepted to the imported tip.
...
// flag-day / production pod spec keeps --import-chain-data SET, so every restart and every G9
// kill-rejoin re-enters the startup import path on an already-imported chain; treating that
// "nothing to import" result as fatal would CRASH-LOOP the node.
//
