...
if [[ -n "${VERSION}" ]]; then
  echo "Ensuring Lux version $VERSION in go.mod"
...
CURL_URL="https://api.github.com/repos/luxfi/node/commits/${LUX_VERSION}"
FULL_LUX_VERSION="$("${CURL_ARGS[@]}" "${CURL_URL}" | grep '"sha":' | head -n1 | cut -d'"' -f4)"
...
echo "Ensuring Lux version ${FULL_LUX_VERSION} for ${CUSTOM_ACTION} custom action in ${WORKFLOW_PATH} "
