...
var _ interfaces.State = (*State)(nil)
...
type State struct {
	interfaces.State
	mySubnetID                   interfaces.ID
	sourceChainID                interfaces.ID
	requirePrimaryNetworkSigners bool
}

// NewState returns a wrapper of [interfaces.State] which special cases the handling of the Primary Network.
//
...
func NewState(state interfaces.State, mySubnetID interfaces.ID, sourceChainID interfaces.ID, requirePrimaryNetworkSigners bool) *State {
	return &State{
		State:                        state,
...
func (s *State) GetValidatorSet(
	ctx context.Context,
	height uint64,
	subnetID interfaces.ID,
) (map[interfaces.NodeID]*interfaces.GetValidatorOutput, error) {
