...
import (
	...
	"github.com/luxfi/evm/v2/stateupgrade"
)
...
// transition from `parentTimestamp` to the timestamp set in `blockContext`. If this is the case, it calls [registry.Module]'s Configure
...
func ApplyPrecompileActivations(c *params.ChainConfig, parentTimestamp *uint64, blockContext contract.ConfigurationBlockContext, statedb *state.StateDB) error {
	...
	// Note: [registry.RegisteredModules] returns precompiles sorted by module addresses.
	// This ensures:
	// - the order we call [registry.Module]'s Configure for each precompile is consistent
		...
		for _, activatingConfig := range extra.GetActivatingPrecompileConfigs(module.Address(), parentTimestamp, blockTimestamp, extra.PrecompileUpgrades) {
			...
			if configurator := module.Configurator(); configurator != nil {
				if conf, ok := configurator.(contract.Configurator); ok {
					if err := conf.Configure(params.GetExtra(c), activatingConfig, extstatedb, blockContext); err != nil {
