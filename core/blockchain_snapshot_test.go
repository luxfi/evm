...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	// For testing, use a memory database
	...
	chain, err := NewBlockChain(db, DefaultCacheConfigWithScheme(basic.scheme), gspec, engine, vm.Config{}, common.Hash{}, false, nil)
	if err != nil {
	...
	for i, block := range blocks {
		if _, err := chain.InsertChain(types.Blocks{block}); err != nil {
			t.Fatalf("Failed to import block %d: %v", i, err)
		}

		// Check if we've reached a snapshot point
		if basic.snapshotBlock > 0 && uint64(i+1) == basic.snapshotBlock {
			// Flushing from 0 to snapshotBlock into the disk
			for j := 0; j <= i; j++ {
				if err := chain.Accept(blocks[j]); err != nil {
					t.Fatalf("Failed to accept block %v: %v", j, err)
				}
				basic.lastAcceptedHash = blocks[j].Hash()
			}
			chain.DrainAcceptorQueue()

			// Wait for snapshot generation to complete
			if chain.snaps != nil {
				diskRoot, blockRoot := chain.snaps.DiskRoot(), blocks[i].Root()
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, DefaultCacheConfigWithScheme(snaptest.scheme), snaptest.gspec, snaptest.engine, vm.Config{}, snaptest.lastAcceptedHash, false, nil)
	if err != nil {
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(db, DefaultCacheConfigWithScheme(snaptest.scheme), snaptest.gspec, snaptest.engine, vm.Config{}, snaptest.lastAcceptedHash, false, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, cacheConfig, snaptest.gspec, snaptest.engine, vm.Config{}, snaptest.lastAcceptedHash, false, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, config, snaptest.gspec, snaptest.engine, vm.Config{}, snaptest.lastAcceptedHash, false, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	...
	tmp, err := NewBlockChain(snaptest.db, config, snaptest.gspec, snaptest.engine, vm.Config{}, snaptest.lastAcceptedHash, false, nil)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
