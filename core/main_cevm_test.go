...
// cevm shadow-verifier counters. Because core/state_processor.go routes EVERY
// block through parallel.DefaultExecutor().ExecuteBlock — which under -tags cevm
// is the cevmShadowExecutor — these counters are the honest, whole-suite tally
// of how many real test blocks cevm's independent real-MPT process_block root
// byte-matched the Go EVM's header.Root. It NEVER applies state: the shadow
// always returns (nil,nil), so the Go path applies every block and the suite
// result is unaffected by cevm.
//
...
func TestMain(m *testing.M) {
	...
	fmt.Printf("\n===== CEVM SHADOW VERIFIER — whole ./core suite tally =====\n")
