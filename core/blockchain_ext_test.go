...
import (
	...
	"github.com/luxfi/evm/v2/v2/upgrade/ap4"
...
// TODO: Add proper callback support when consensus engine supports it
// For now, these tests will use the default dummy engine

type createFunc func(db ethdb.Database, gspec *Genesis, lastAcceptedHash common.Hash, dataPath string) (*BlockChain, error)
type ExtChainTest struct {
	Name     string
	testFunc func(
		t *testing.T,
		create createFunc,
...
func copyMemDBExt(db ethdb.Database) (ethdb.Database, error) {
...
func checkBlockChainStateExt(
	...
	originalDB, err = copyMemDBExt(originalDB)
	if err != nil {
		t.Fatal(err)
	}
...
func InsertChainAcceptSingleBlockTest(t *testing.T, create createFunc) {
	...
	checkBlockChainStateExt(t, blockchain, gspec, chainDB, create, checkState)
}
...
func SetPreferenceRewindTest(t *testing.T, create createFunc) {
	...
	checkBlockChainStateExt(t, blockchain, gspec, chainDB, create, checkGenesisState)
	...
	checkBlockChainStateExt(t, blockchain, gspec, chainDB, create, checkUpdatedState)
}
...
func ReexecBlocksTest(t *testing.T, create ReexecTestFunc) {
	...
	_, newChain, restartedChain := checkBlockChainStateExt(t, blockchain, gspec, chainDB, checkCreate, checkState)
