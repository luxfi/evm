...
func TestVerifyPrecompileUpgradeConfig(t *testing.T) {
...
func TestCheckCompatiblePrecompileUpgradeConfigs(t *testing.T) {
	admins := []common.Address{{1}}
	chainConfig := &extras.ChainConfig{}
	chainConfig.GenesisPrecompiles = extras.Precompiles{
		txallowlist.ConfigKey:       txallowlist.NewConfig(utils.NewUint64(1), admins, nil, nil),
	...
	tests := map[string]upgradeCompatibilityTest{
		...
		"retroactively enabling upgrades is not allowed": {
			expectedErrorString: "cannot retroactively enable PrecompileUpgrade[1] in database (have timestamp nil, want timestamp 5, rewindto timestamp 5)",
