...
var (
	...
	_                     nodeinterfaces.AppHandler = &network{}
)

type Network interface {
	validators.Connector
	nodeinterfaces.AppHandler
...
type network struct {
	...
	appSender                  nodeinterfaces.AppSender                   // node AppSender for sending messages
...
func NewNetwork(p2pNetwork *p2p.Network, appSender nodeinterfaces.AppSender, codec codec.Manager, self ids.NodeID, maxActiveAppRequests int64) Network {
	return &network{
		appSender:                  appSender,
		codec:                      codec,
		self:                       self,
...
func (n *network) AppRequestFailed(ctx context.Context, nodeID ids.NodeID, requestID uint32, appErr *nodeinterfaces.AppError) error {
